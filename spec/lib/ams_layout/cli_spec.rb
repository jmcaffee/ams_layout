...
describe 'ams_layout executable' do
  ...
  context "help commands" do
    it "displays help info" do
      ...
      expect( output ).to include "layout [PATH]"
      expect( output ).to include "cls <opts> [PATH] [LAYOUT_PATH]"
      expect( output ).to include "delegate <opts> [PATH] [LAYOUT_PATH]"
      expect( output ).to include "all <opts> [PATH]"
    end
